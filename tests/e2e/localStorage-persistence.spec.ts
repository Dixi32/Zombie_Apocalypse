...
test.describe('localStorage persistence E2E tests', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForFunction(
      () => window.i18n !== undefined && window.theme !== undefined
    );
  ...
  test.describe('Cross-page persistence', () => {
    ...
    test('should persist preferences across page reloads', async ({ page }) => {
      ...
      await page.waitForFunction(
        () =>
          window.testGetLanguage !== undefined &&
          window.testGetTheme !== undefined
      );
  ...
  test.describe('User interaction scenarios', () => {
    test('should maintain preferences during typical user workflow', async ({
      page,
    }) => {
  ...
  test.describe('Error handling and recovery', () => {
    test('should handle localStorage quota exceeded gracefully', async ({
      ...
      const hasStorageErrors = consoleMessages.some(
        (msg) => msg.includes('Error saving') && msg.includes('localStorage')
      );
    ...
    test('should recover from corrupted localStorage data', async ({
      ...
      const hasWarnings = consoleMessages.some(
        (msg) =>
          msg.includes('Invalid') &&
          (msg.includes('language') || msg.includes('theme'))
      );
